...
if __name__ == "__main__":
    ...
    print(f"ADC resolution: {processor.bits} bits")
    print(f"Test signal: 1 kHz sine wave, {duration} seconds")

    # Process through complete pipeline
    compressed, num_samples = processor.process_for_transmission(test_signal)
    reconstructed = processor.process_for_playback(compressed, num_samples)

    # Calculate metrics
    snr = processor.calculate_snr(test_signal, reconstructed)
    compression_ratio = (num_samples * 2) / len(compressed)  # 16-bit to bytes

    print(f"\nResults:")
    print(f"Original size: {num_samples * 2} bytes")
